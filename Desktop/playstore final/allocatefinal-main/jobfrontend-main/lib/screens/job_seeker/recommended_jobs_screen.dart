...
/// Full list of jobs recommended based on the seeker's profile.
...
class _RecommendedJobsScreenState extends State<RecommendedJobsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final jobs = await _api.getRecommendedJobs(page: 1, perPage: 15);
  ...
  Future<void> _loadMore() async {
    ...
    try {
      ...
      if (_isFallback || !AppSession.isLoggedIn) {
        ...
        jobs = await _api.getRecommendedJobs(page: nextPage, perPage: 15);
      }
  ...
  Future<void> _refreshAppliedIds() async {
    ...
    try {
      final apps = await _api.listMyApplications();
  ...
  Future<void> _loadSavedJobIds() async {
    ...
    try {
      final saved = await _api.listSavedJobs();
            ...
            child: Container(
              ...
              child: const Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Complete your profile to get personalised picks. Showing all jobs for now.',
                      style: TextStyle(
                          fontSize: 12,
