...
class _JobSeekerProfileEditSheetState extends State<JobSeekerProfileEditSheet> {
  ...
  bool _generatingHeadline = false;
  final Map<int, bool> _loadingProjectAI = {};
  final Map<int, bool> _loadingInternshipAI = {};

  Future<void> _generateHeadlineWithAI() async {
    final skills = _skillsCtrl.text.trim();
    final bio = _bioCtrl.text.trim();
    if (skills.isEmpty && bio.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please add some skills or a bio first so AI has context!'),
    ...
    setState(() => _generatingHeadline = true);
    try {
      final text = await JobSeekerApiService.instance.resumeAiAssist(
        sectionName: 'professional_headline',
        currentText: _headlineCtrl.text,
        instruction: 'Generate a short, catchy, professional 1-line headline based on my profile details. Output ONLY the headline text (under 10 words). No quotes, no preamble.',
        jobContext: 'My skills: $skills. My bio: $bio',
      );
      setState(() {
        _headlineCtrl.text = text;
      });
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Headline generated by AI!'),
      ...
      if (mounted) setState(() => _generatingHeadline = false);
    }
  }
  ...
  Future<void> _generateBioWithAI() async {
    ...
    final skills = _skillsCtrl.text.trim();
    if (headline.isEmpty && skills.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please add some skills or a headline first so AI has context!'),
          backgroundColor: AppColors.error,
        ),
      );
      return;
    ...
    try {
      final text = await JobSeekerApiService.instance.resumeAiAssist(
        sectionName: 'bio',
        currentText: _bioCtrl.text,
        instruction: 'Generate a professional, compelling 2-3 sentence bio/summary. Focus on my professional identity, skills, and value. Output ONLY the bio text. No quotes, no preamble.',
        jobContext: 'My headline: $headline. My skills: $skills.',
      );
      setState(() {
        _bioCtrl.text = text;
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Bio generated by AI!'),
          backgroundColor: AppColors.success,
        ),
      );
    } catch (e) {
  ...
  Future<void> _generateProjectInfoWithAI(int index) async {
    final project = _projects[index];
    final title = project.title.text.trim();
    if (title.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    setState(() => _loadingProjectAI[index] = true);
    try {
      final text = await JobSeekerApiService.instance.resumeAiAssist(
        sectionName: 'project_description',
        currentText: project.description.text,
        instruction: 'Generate a short, professional 2-3 bullet point description for a project named "$title". Focus on key features and technologies used. Output ONLY the bullet list, no introductory phrase.',
      );
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Project info generated by AI!'),
          backgroundColor: AppColors.success,
        ),
      );
    } catch (e) {
      ...
      if (mounted) setState(() => _loadingProjectAI[index] = false);
    }
  }

  Future<void> _generateInternshipInfoWithAI(int index) async {
    final internship = _internships[index];
    final company = internship.organization.text.trim();
    final role = internship.role.text.trim();
    if (company.isEmpty || role.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please enter Role and Organization first!'),
          backgroundColor: AppColors.error,
        ),
      );
      return;
    ...
    setState(() => _loadingInternshipAI[index] = true);
    try {
      final text = await JobSeekerApiService.instance.resumeAiAssist(
        sectionName: 'internship_description',
        currentText: internship.description.text,
        instruction: 'Generate a professional 2-3 bullet point description of roles and responsibilities for an internship as a "$role" at "$company". Output ONLY the bullet list.',
      );
      setState(() {
        internship.description.text = text;
      });
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Internship info generated by AI!'),
          backgroundColor: AppColors.success,
        ),
      );
    } catch (e) {
      ...
      if (mounted) setState(() => _loadingInternshipAI[index] = false);
    }
  }
  ...
  void initState() {
    ...
    final rawDob = p['dob']?.toString() ?? '';
    _dobCtrl = TextEditingController(
      text: rawDob.isNotEmpty ? rawDob.split(' ').first.split('T').first : '',
    );
  ...
  void _addAchievement() => setState(() => _achievements.insert(0, _AchievementRow()));
  ...
  void _addInternship() => setState(() => _internships.insert(0, _InternshipRow()));
  ...
  void _addProject() => setState(() => _projects.insert(0, _ProjectRow()));
  ...
  void _addEducation() {
    setState(() => _education.insert(0, _EduRow()));
  }
  ...
  void _addWorkExp() => setState(() => _workExperience.insert(0, _WorkExpRow()));
  ...
  void _addLanguage() => setState(() => _languages.insert(0, _LanguageRow()));
  ...
  void _addCertification() => setState(() => _certifications.insert(0, _CertificationRow()));
  ...
  void _addAcademicAchievement() => setState(() => _academicAchievements.insert(0, _AcademicAchievementRow()));
  ...
  void _addAward() => setState(() => _awards.insert(0, _AwardsRow()));
  ...
  void _addExamResult() => setState(() => _examResults.insert(0, _ExamResultRow()));
                  ...
                  Row(
                    children: [
                      ...
                      _generatingHeadline
                          ...
                          : IconButton(
                              onPressed: _generateHeadlineWithAI,
                            ...
                            Row(
                              children: [
                                ...
                                (_loadingInternshipAI[i] ?? false)
                                    ...
                                    : IconButton(
                                        onPressed: () => _generateInternshipInfoWithAI(i),
                                        icon: const Icon(Icons.auto_awesome, color: AppColors.primary),
                            ...
                            Row(
                              children: [
                                ...
                                (_loadingProjectAI[i] ?? false)
                                    ...
                                    : IconButton(
                                        onPressed: () => _generateProjectInfoWithAI(i),
                                        icon: const Icon(Icons.auto_awesome, color: AppColors.primary),
                                        tooltip: 'Write with AI',
                                      ),
                              ],
                          ...
                          children: [
                            Expanded(
                              child: Column(
                                children: [
                                  DropdownButtonFormField<String>(
                                    value: kCommonLanguages.contains(r.language.text) ? r.language.text : null,
                                    ...
                                    items: kCommonLanguages.map((lang) => DropdownMenuItem(
                                      value: lang,
                                      child: Text(lang),
                                    )).toList(),
                                    onChanged: (val) {
                                      if (val != null) {
                                        setState(() => r.language.text = val);
                                      }
                                    },
                                  ),
                                  const SizedBox(height: 8),
                                  DropdownButtonFormField<String>(
                                    value: (r.proficiency.text == 'Fluent' || r.proficiency.text == 'Native')
                                        ? r.proficiency.text
                                        : null,
                                    hint: const Text('Select'),
                                    isExpanded: true,
                                    ...
                                    onChanged: (val) {
                                      if (val != null) {
                                        setState(() => r.proficiency.text = val);
                                      }
                                    },
                                  ),
                                ],
